...
class TokenReceiver:
    ...
    def __init__(
        ...
        if os.getenv("VKPYMUSIC_DISABLE_LOGGING", "False") != "True":
            self.set_logger()

    def set_logger(self, logger: Union[logging.Logger, False] = None) -> None:
        """
    ...
    #################
    # PRIVATE METHODS
    def __log(self, msg: str, level: str = "info"):
        if self.__logger \
            ...
            try:
                ...
                # 2FA is needed
                    ...
                    # Other type of 2FA
                        ...
                        self.__log("Please, create an issue in repository for adding this type.")
                ...
                # Invalid code for 2FA
                    ...
                    self.__log(level="warning", msg="Invalid code. Try again!")
                    code = on_2fa()
                # Login or password is invalid
                elif error == "invalid_client":
                    self.__log(level="error", msg="Login or password is invalid!")
                    del self.__login
                    del self.__password
                    on_invalid_client()
                    return False
                # Many unsuccessful attempts
                    ...
                    self.__log(level="error", msg="Password bruteforce attempt!")
                    del self.__login
                    del self.__password
                    return False
                # Undefined error
    ...
    def get_token(self) -> Optional[str]:
        ...
        token = self.__token
        if not token:
            self.__log(level="warning", msg='Please, first call the method "auth".')
            return
        self.__log(token)
        return token
    ...
    def save_to_config(self, file_path: str = "config_vk.ini"):
        ...
        if os.path.isfile(full_fp):
            self.__log(level="warning", msg="File already exist! It will be overwritten.")
